...
export const fetchPokemonDetail = async (name: string):Promise<PokemonDetailType> => {
  ...
  const speciesResponse = await remote.get<PokemonSpeciesResponseType>(pokemonSpeciesUrl) //한국어 api
  ...
  return {
    ...
    height: detail.height / 10, // m단위로 변환하기에 나누기 10
    weight: detail.weight / 10, // kg단위로 변환하기에 나누기 10
